...
def source_text(lead: dict[str, Any], city: str = "") -> str:
    """Everything we actually know, lowercased, for the fabrication check."""
    return facts_for(lead, city).lower()
...
_DURATION = re.compile(r"\b(?:\d+|one|two|three|four|five|ten|twenty|thirty|forty|fifty)"
                       r"[\s-]*(?:\+\s*)?(?:years?|decades?|generations?)\b", re.I)
_HTML = re.compile(r"<[^>]+>")
...
# Plumbing" may say so.
CREDENTIAL_TERMS = [
    "licensed", "licence", "license", "insured", "bonded", "certified",
    ...
    "established", "founded", "since", "generations", "guarantee",
...
def review(content: dict[str, Any], source: str) -> list[str]:
    ...
    for field_name, text in _strings(content):
        ...
        for match in _DURATION.findall(text):
            issues.append(f"{field_name} claims a length of time: '{match.strip()}'")
...
def write_copy(lead: dict[str, Any], city: str = "",
               ...
               max_attempts: int = 2) -> tuple[dict[str, Any], Usage]:
    ...
    raise ContentError(
        "The copy still breaks the rules after a retry, so nothing was written:\n"
        + "\n".join(f"  - {i}" for i in issues[:12])
        + "\n\nThis usually means the business name itself trips a check. Either "
          "build this one by hand or adjust the term list in generate.py."
    )
...
def render(lead: dict[str, Any], content: dict[str, Any], *,
    ...
    return Site(html=html, robots=robots, template=template, palette=palette)
